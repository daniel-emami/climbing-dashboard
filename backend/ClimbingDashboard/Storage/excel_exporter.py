...
class ExcelBoulderExporter:
    ...
    def build_workbook(self, boulders: list[BoulderRecord]) -> BytesIO:
        ...
        for row_index, boulder in enumerate(boulders, start=2):
            ...
            worksheet.cell(row_index, 6, boulder.sector)
            worksheet.cell(row_index, 7, 1 if boulder.flash else 0)
            date_cell = worksheet.cell(row_index, 8, boulder.climbed_on)
            date_cell.number_format = "yyyy-mm-dd"
            worksheet.cell(row_index, 9, boulder.climber)
            worksheet.cell(row_index, 10, boulder.rating)
            worksheet.cell(row_index, 11, boulder.visibility)
